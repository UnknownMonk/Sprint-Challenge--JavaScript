...
const universities = [];
const school = graduates.map(school => school.university);
...
const info = graduates.map(contact => {
...
for (i = 0; i < graduates.length; i++) {
  uniStr.push(graduates[i].university);
}

for (i = 0; i < uniStr.length; i++) {
  if (uniStr[i].search('Uni') > -1) {
    uni.push(uniStr[i]);
  }
}
console.log(uni);
...
zooAnimals.forEach(name => {
  let regName = ` Name :${name.animal_name} Scientific: ${
    name.scientific_name
  } `;
  animalNames.push(regName);
});
...
const lowerCase = [];
const animalArr = zooAnimals.map(names => names.animal_name.toLowerCase());
lowerCase.push(animalArr);
...
const smallPoP = zooAnimals.filter(pop => pop.population < 5);
largerPopulation.push(smallPoP);
